...
//Necesary for connecting to the web server
const char* ssid = "iPhone - Matei";     // CHANGE THIS
const char* password = "clujCalacea2021"; // CHANGE THIS
...
bool parseConfig(String jsonPayload) {
  ...
  intersection_init(&intr, doc["defaultPhaseDurationMs"]);

  JsonArray lanes = doc["lanes"];
    for (JsonObject l : lanes) {
        LaneHardware hw;
        hw.sensor_pin = l["sensorPin"];
        hw.green_pin  = l["greenPin"];
        hw.yellow_pin = l["yellowPin"];
        hw.red_pin    = l["redPin"];

        // Note: l["type"] comes as 0 or 1 from C# Enum
        add_lane(&intr, l["localId"], (LaneType)l["type"].as<int>(), hw);
    }

    JsonArray connections = doc["connections"];
    for (JsonObject c : connections) {
        // C# sends "Indices" (0, 1, 2) which map directly to our array
        add_connection(&intr, c["sourceLaneIdx"], c["targetLaneIdx"]);
    }

    JsonArray phases = doc["phases"];
    for (JsonObject p : phases) {
        uint64_t mask = 0;
        JsonArray indices = p["activeConnectionIndices"];
        
        // Convert List<int> [0, 2] -> Bitmask 101
        for (int connIdx : indices) {
             mask |= (1ULL << connIdx);
        }
        
        add_phase(&intr, mask, p["durationMs"]);
    }
...
void setup() {
  ...
  if(httpCode == 200) {
    ...
    if (parseConfig(payload)) {
      Serial.println("✅ Config Success! Starting Traffic Logic.");
      configured = true; // BREAK THE LOOP
                
        // Set PinModes based on the new Config
      for(int i=0; i<intr.lane_cnt; i++) {
        LaneHardware h = intr.lanes[i].hw;
        if(h.red_pin >= 0) pinMode(h.red_pin, OUTPUT);
        if(h.yellow_pin >= 0) pinMode(h.yellow_pin, OUTPUT);
        if(h.green_pin >= 0) pinMode(h.green_pin, OUTPUT);
        if(h.sensor_pin >= 0) pinMode(h.sensor_pin, INPUT);
      }
    }
